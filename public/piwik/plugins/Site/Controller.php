 ...
 * @package Piwik_SitesManager
 */
class Piwik_Site_Controller extends Piwik_Controller 
{
	...
	public function getSitesIdFromSiteUrl( $url )
	{
		...
		else
		{
			...
			$ids = Zend_Registry::get('db')->fetchOne(
					'SELECT idsite 
					FROM ' . Piwik_Common::prefixTable('site') . ' 
					WHERE main_url = ? ', $url);
		}
	 ...
	 */
	public function addSite( $siteName, $urls, $timezone = null, $currency = null, $excludedIps = null, $excludedQueryParameters = null )
	{
		//Piwik::checkUserIsSuperUser();
