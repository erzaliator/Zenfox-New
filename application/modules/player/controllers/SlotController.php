...
class Player_SlotController extends Zenfox_Controller_Action
{
	...
	public function gamelogAction()
	{
		...
		if($this->getRequest()->isPost())
		{
			...
			$result = $slot->showLog($data['page'], $offset, $fromDate, $toDate);
			if(!$result)
			{
				$this->_helper->FlashMessenger(array('error' => 'No slot record found.'));
			}
		...
		elseif($offset)
		{
			...
			$result = $slot->showLog($itemsPerPage, $offset, $from, $to);
			if(!$result)
			{
				$this->_helper->FlashMessenger(array('error' => 'No slot record found.'));
			}
