...
class Player_KenoController extends Zenfox_Controller_Action
{	
	...
	public function gamelogAction()
	{
		...
		if($this->getRequest()->isPost())
		{
			...
			$result = $keno->showLog($data['page'], $offset, $fromDate, $fromDate);
			if(!$result)
			{
		...
		elseif($offset)
		{
			...
			$result = $keno->showLog($itemsPerPage, $offset, $from, $to);
			if(!$result)
			{
				$this->_helper->FlashMessenger(array('error' => 'No keno record found.'));
			}
