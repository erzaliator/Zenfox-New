<?php
class KenoLog extends Doctrine_Record
{
	private $_playerId;
...
//		Zenfox_Debug::dump($paginator, 'paginator', true, true);
		if($paginator->getTotalItemCount())
		{
			...
			{
				$kenoLogs[$index][$translate->translate('Gamelog Id')] = $logs['session_id'] . '-' . $logs['log_id'];
				//$kenoLogs[$index][$translate->translate('Session Id')] = $logs['session_id'];
				$kenoLogs[$index][$translate->translate('Machine Name')] = $logs->RunningKeno['machine_name'];
				$kenoLogs[$index][$translate->translate('Currency')] = $logs['wagered_currency'];
				//$kenoLogs[$index][$translate->translate('Win Amount')] = $logs['win_amount'];
				$kenoLogs[$index][$translate->translate('Amount Type')] = $logs['amount_type'];
				$kenoLogs[$index][$translate->translate('Pjp Win Status')] = $logs['pjp_winstatus'];
				//$kenoLogs[$index][$translate->translate('Pjp Win Amount')] = $logs['pjp_win_amount'];
				//$kenoLogs[$index]['Date & Time'] = $logs['datetime'];
				$kenoLogs[$index][$translate->translate('Bet Amount')] = $currency->setCurrency($logs['wagered_currency'], $logs['bet_amount']);
				$kenoLogs[$index][$translate->translate('Win Amount')] = $currency->setCurrency($logs['wagered_currency'], $logs['win_amount']);
				$kenoLogs[$index][$translate->translate('Pjp_Win Amount')] = $currency->setCurrency($logs['wagered_currency'], $logs['pjp_win_amount']);
				$kenoLogs[$index][$translate->translate('Date & Time')] = $logs['datetime']; //$date->setDate($logs['datetime']);
				$kenoLogs[$index][$translate->translate('Outcome')] = '';
				$index++;
			}
			$paginatorSession->unsetAll();
			return array($paginator, $kenoLogs);
		}
		return NULL;
